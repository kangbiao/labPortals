...
<head>
    <meta charset="UTF-8">
    <title><?php echo ($meta_title); ?>|教研室后台管理系统</title>
...
<body>
    ...
    <div class="header">
        <!-- Logo -->
        <span class="logo"><font color="white" style="font-size:20px">XXX教研室后台管理</font></span>
        <!-- /Logo -->
 ...
 <!--</h3>-->
 	<!--<ul class="side-sub-menu <?php if(!in_array((ACTION_NAME), explode(',',"mydocument,draftbox,examine"))): ?>subnav-off<?php endif; ?>">-->
 		...
 		<!--<li <?php if((ACTION_NAME) == "draftbox"): ?>class="current"<?php endif; ?>><a class="item" href="<?php echo U('article/draftbox');?>">草稿箱</a></li>-->
 		<!--<?php endif; ?>-->
		<!--<?php if(($show_examine) == "1"): ?>-->
		<!--<li <?php if((ACTION_NAME) == "draftbox"): ?>class="examine"<?php endif; ?>><a class="item" href="<?php echo U('article/examine');?>">待审核</a></li>-->
		<!--<?php endif; ?>-->
 	<!--</ul>-->
    <?php if(is_array($nodes)): $i = 0; $__LIST__ = $nodes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub_menu): $mod = ($i % 2 );++$i; if(empty($sub_menu['_child'])): ?><h3>
                &nbsp;&nbsp;&nbsp;&nbsp;
            ...
            <ul></ul><?php endif; ?>
        <!-- 子导航 -->
        <?php if(!empty($sub_menu['_child'])): ?><h3 class="current">
